...
import { ControlPlaneClient, ControlPlaneSessionInfo } from "../control-plane/client.js"
import { getControlPlaneEnv, useHub } from "../control-plane/env.js"
import { BrowserSerializedContinueConfig, ContinueConfig, IContextProvider, IDE, IdeSettings, ILLM } from "../index.js"
import Ollama from "../llm/llms/Ollama.js"
import { GlobalContext } from "../util/GlobalContext.js"
import { getConfigJsonPath, getConfigYamlPath } from "../util/paths.js"
import { localPathToUri } from "../util/pathToUri.js"

import { LOCAL_ONBOARDING_CHAT_MODEL, LOCAL_ONBOARDING_PROVIDER_TITLE } from "./onboarding.js"
...
import { ProfileDescription, ProfileLifecycleManager } from "./ProfileLifecycleManager.js"
...
export class ConfigHandler {
	...
	constructor(
		...
		this.controlPlaneClient = new ControlPlaneClient(sessionInfoPromise, ideSettingsPromise)

		// Set local profile as default
		const localProfileLoader = new LocalProfileLoader(ide, ideSettingsPromise, this.controlPlaneClient, writeLog)
		this.localProfileManager = new ProfileLifecycleManager(localProfileLoader, this.ide)
	...
	get currentProfile() {
		...
		return this.profiles?.find((p) => p.profileDescription.id === this.selectedProfileId) ?? null
	}

	get inactiveProfiles() {
		return (this.profiles ?? []).filter((p) => p.profileDescription.id !== this.selectedProfileId)
	}
	...
	async loadAssistantsForSelectedOrg() {
		...
		if (!userId) {
			...
			// Logged in
			const assistants = await this.controlPlaneClient.listAssistants(selectedOrgId)
	...
	private async updateAvailableProfiles(profiles: ProfileLifecycleManager[]) {
		...
		const previouslySelectedProfileId = await this.getPersistedSelectedProfileId()

		// Check if the previously selected profile exists in the current profiles
		const profileExists = profiles.some((profile) => profile.profileDescription.id === previouslySelectedProfileId)
		...
		// Notify listeners
		const profileDescriptions = profiles.map((profile) => profile.profileDescription)
	...
	private async reloadHubAssistants() {
		const selectedOrgId = await this.getSelectedOrgId()
		const newFullSlugsList = await this.controlPlaneClient.listAssistantFullSlugs(selectedOrgId)

		if (newFullSlugsList) {
			const shouldReload = this.fullSlugsListsDiffer(newFullSlugsList, this.lastFullSlugsList)
			if (shouldReload) {
	...
	async getPersistedSelectedProfileId(): Promise<string | null> {
		...
		const lastSelectedIds = this.globalContext.get("lastSelectedProfileForWorkspace") ?? {}
	...
	async setSelectedProfile(profileId: string | null) {
		...
		this.globalContext.update("lastSelectedProfileForWorkspace", selectedProfiles)
	}
	...
	async updateControlPlaneSessionInfo(sessionInfo: ControlPlaneSessionInfo | undefined) {
		this.controlPlaneClient = new ControlPlaneClient(Promise.resolve(sessionInfo), this.ideSettingsPromise)
	...
	private profilesListeners: ((profiles: ProfileDescription[], selectedProfileId: string | null) => void)[] = []
	onDidChangeAvailableProfiles(listener: (profiles: ProfileDescription[], selectedProfileId: string | null) => void) {
		this.profilesListeners.push(listener)
	}

	private notifyProfileListeners(profiles: ProfileDescription[], selectedProfileId: string | null) {
	...
	async reloadConfig() {
		...
		const { config, errors, configLoadInterrupted } = await this.currentProfile.reloadConfig(
	...
	async loadConfig(): Promise<ConfigResult<ContinueConfig>> {
		...
		return await this.currentProfile.loadConfig(this.additionalContextProviders)
	}
