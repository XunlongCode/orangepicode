...
FROM information_schema.tables`
		...
		return tablesInfo.map((tableInfo: any) => `${tableInfo.table_schema}.${tableInfo.table_name}`)
	}

	async getContextItems(query = "", _: ContextProviderExtras = {} as ContextProviderExtras): Promise<ContextItem[]> {
...
WHERE table_schema = '${tableName.split(".")[0]}'
  AND table_name = '${tableName.split(".")[1]}'`
				...
				// Get the sample rows
				const sampleRows = this.options.sampleRows ?? PostgresContextProvider.DEFAULT_SAMPLE_ROWS
...
LIMIT ${sampleRows}`)
	...
	async loadSubmenuItems(_: LoadSubmenuItemsArgs): Promise<ContextSubmenuItem[]> {
		const pool = await this.getPool()
