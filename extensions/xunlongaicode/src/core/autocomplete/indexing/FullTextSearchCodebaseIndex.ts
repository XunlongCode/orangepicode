...
export class FullTextSearchCodebaseIndex implements CodebaseIndex {
	...
	async *update(
		...
		for (let i = 0; i < results.compute.length; i++) {
			...
			// Insert chunks
			const chunks = await db.all("SELECT * FROM chunks WHERE path = ? AND cacheKey = ?", [
				item.path,
				item.cacheKey,
			])

			for (const chunk of chunks) {
				const { lastID } = await db.run("INSERT INTO fts (path, content) VALUES (?, ?)", [
					item.path,
					chunk.content,
				])
				await db.run(
      ...
      `,
			...
			await db.run("DELETE FROM fts_metadata WHERE path = ? AND cacheKey = ?", [item.path, item.cacheKey])
	...
	private getRetrieveQueryParameters(config: RetrieveConfig) {
		...
		return [text.replace(/\?/g, ""), ...tagStrings, ...(filterPaths || []), Math.ceil(n)]
	}
