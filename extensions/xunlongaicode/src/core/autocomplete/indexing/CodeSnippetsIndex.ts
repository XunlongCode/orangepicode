...
export class CodeSnippetsCodebaseIndex implements CodebaseIndex {
	...
	async getSnippetsInFile(filepath: string, contents: string): Promise<SnippetChunk[]> {
		...
		const query = await getQueryForFile(filepath, `code-snippet-queries/${language}.scm`)
	...
	async *update(
		...
		for (let i = 0; i < results.del.length; i++) {
			...
			const snippets = await db.all("SELECT id FROM code_snippets WHERE path = ? AND cacheKey = ?", [
				del.path,
				del.cacheKey,
			])
	...
	static async getForId(id: number, workspaceDirs: string[]): Promise<ContextItem> {
		const db = await SqliteDb.get()
