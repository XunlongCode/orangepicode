...
export async function getModelQuickPickVal(curModelTitle: string, config: ContinueConfig) {
	const modelItems: QuickPickItem[] = config.models.map((model) => {
		const isCurModel = curModelTitle === model.title

		return {
			label: model.title ? `${isCurModel ? "$(check)" : "     "} ${model.title}` : "Model title not set",
		}
	})
