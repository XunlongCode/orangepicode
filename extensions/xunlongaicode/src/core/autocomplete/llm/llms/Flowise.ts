...
class Flowise extends BaseLLM {
	...
	protected _getRequestBody(messages: ChatMessage[], options: CompletionOptions): any {
		...
		const history = messages
			...
			.map((m) => ({
				type: m.role === "user" ? Flowise.FlowiseMessageType.User : Flowise.FlowiseMessageType.Assistant,
				message: m.content,
			}))
		const requestBody = {
	...
	protected _initializeSocket(): Promise<{
		...
		return new Promise((res, rej) => {
			...
			socketInfo.hasNextToken = () => {
				return new Promise<boolean>((hasNextTokenResolve, hasNextTokenReject) => {
					socketInfo.internal.hasNextTokenPromiseResolve = hasNextTokenResolve
