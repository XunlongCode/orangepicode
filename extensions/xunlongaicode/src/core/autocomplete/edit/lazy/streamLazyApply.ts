...
export async function* streamLazyApply(
	...
	async function* replacementFunction(
		...
		for await (const line of getReplacementWithLlm(oldCode, linesBefore, linesAfter, fastLlm)) {
			yield line
		}
	}
	...
	let lines = streamFillUnchangedCode(lazyCompletionLines, oldCode, replacementFunction)
...
async function* streamFillUnchangedCode(
	lines: LineStream,
	oldCode: string,
	replacementFunction: (oldCode: string, linesBefore: string[], linesAfter: string[]) => AsyncGenerator<string>,
): LineStream {
	const newLines = []
	let buffer = []
