...
import { EXTENSION_NAME, getControlPlaneEnvSync } from "../control-plane/env"
...
const controlPlaneEnv = getControlPlaneEnvSync(true ? "production" : "none", enableControlServerBeta)
...
async function generateCodeChallenge(verifier: string) {
	...
	const base64String = hash.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "")
...
export class WorkOsAuthProvider implements AuthenticationProvider, Disposable {
	...
	constructor(
		...
		this._disposable = Disposable.from(
			authentication.registerAuthenticationProvider(controlPlaneEnv.AUTH_TYPE, AUTH_NAME, this, {
	...
	private decodeJwt(jwt: string): Record<string, any> | null {
		try {
			const decodedToken = JSON.parse(Buffer.from(jwt.split(".")[1], "base64").toString())
			return decodedToken
		} catch (e: any) {
	...
	public async getSessions(scopes?: string[]): Promise<ContinueAuthenticationSession[]> {
	...
	get ideRedirectUri() {
		if (env.uriScheme === "vscode-insiders" || env.uriScheme === "vscode" || env.uriScheme === "code-oss") {
	...
	private async _refreshSessions(): Promise<void> {
		...
		for (const session of sessions) {
			try {
				...
				await this.debugAccessTokenValidity(session.accessToken, session.refreshToken)
	...
	private async _refreshSession(refreshToken: string): Promise<{
		...
		const response = await fetch(new URL("/auth/refresh", controlPlaneEnv.CONTROL_PLANE_URL), {
			method: "POST",
			headers: {
	...
	private _formatProfileLabel(firstName: string | null, lastName: string | null) {
	 ...
	 * @returns
	 */
	public async createSession(scopes: string[]): Promise<ContinueAuthenticationSession> {
		try {
			...
			setTimeout(() => this._refreshSessions(), (this.getExpirationTimeMs(session.accessToken) * 2) / 3)
			...
			async (_, token) => {
				...
				try {
					return await Promise.race([
						...
						new Promise<string>(
							(_, reject) => setTimeout(() => reject("Cancelled"), 60 * 60 * 1_000), // 60min timeout
						),
