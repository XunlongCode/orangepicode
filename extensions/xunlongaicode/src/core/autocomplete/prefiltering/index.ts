...
async function isDisabledForFile(currentFilepath: string, disableInFiles: string[] | undefined, ide: IDE) {
	if (disableInFiles) {
...
export async function shouldPrefilter(helper: HelperVars, ide: IDE): Promise<boolean> {
