...
class DocsCrawler {
	...
	private shouldUseChromium() {
		return this.config.experimental?.useChromiumForDocsCrawling && this.chromiumInstaller.isInstalled()
	}

	/*
    Returns the type of crawler used in the end
  */
	async *crawl(startUrl: URL): AsyncGenerator<PageData, DocsCrawlerType, undefined> {
		...
		if (this.shouldUseChromium()) {
			...
			const shouldProposeUseChromium =
				!didCrawlSinglePage && this.chromiumInstaller.shouldProposeUseChromiumOnCrawlFailure()
