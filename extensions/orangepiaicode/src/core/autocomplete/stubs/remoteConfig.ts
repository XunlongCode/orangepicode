...
import { getConfigJsonPathForRemote } from "../util/paths"
...
export class RemoteConfigSync {
	...
	constructor(
		...
		let { userToken: settingsUserToken, remoteConfigServerUrl, remoteConfigSyncPeriod } = this.loadVsCodeSettings()
		...
		vscode.workspace.onDidChangeConfiguration(async (event) => {
			if (event.affectsConfiguration(CONTINUE_WORKSPACE_KEY)) {
				const { userToken, remoteConfigServerUrl, remoteConfigSyncPeriod } = await this.loadVsCodeSettings()
				if (
	...
	private loadVsCodeSettings() {
		...
		const remoteConfigServerUrl = settings.get<string | null>("remoteConfigServerUrl", null)
		const remoteConfigSyncPeriod = settings.get<number>("remoteConfigSyncPeriod", 60)
	...
	async sync(userToken: string, remoteConfigServerUrl: string) {
		try {
			const client = new ContinueServerClient(remoteConfigServerUrl.toString(), userToken)
			const { configJson } = await client.getConfig()

			fs.writeFileSync(getConfigJsonPathForRemote(remoteConfigServerUrl), configJson)
