...
import { ChatMessage, CompletionOptions } from "../../core/autocomplete"
import { PROVIDER_TOOL_SUPPORT } from "../../core/autocomplete/llm/toolSupport"
import { renderChatMessage } from "../../core/autocomplete/util/messageContent"
...
3. The region in the ARN matches the region where you're making the request`,
	...
	protected async *_createComplete(
		...
		const input = this._generateConverseInput(messages, { ...completionOptions, stream: true })
			...
			for await (const chunk of response.stream) {
				if (chunk.contentBlockDelta?.delta) {
					...
					if (chunk.contentBlockDelta.delta.text) {
						yield renderChatMessage({ role: "assistant", content: chunk.contentBlockDelta.delta.text })
						continue
					}
