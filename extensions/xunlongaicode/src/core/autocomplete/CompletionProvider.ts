...
import { ApiHandler, buildApiHandler } from "../../api/index.js"
import { ApiConfiguration } from "../../shared/api.js"

// TODO 去掉缓存相关代码，这部分代码有报错，造成插件启动不起来
...
export class CompletionProvider {
	...
	constructor(
		...
		autocompleteApiConfiguration: ApiConfiguration,
		private readonly _onError: (e: any) => void,
		private readonly getDefinitionsFromLsp: GetLspDefinitionsFunction,
	) {
		this.apiHandler = buildApiHandler(autocompleteApiConfiguration)
	...
	private async _getAutocompleteOptions() {
		// const config = await this.configManager.listConfig()
	...
	public async provideInlineCompletionItems(
		...
		console.log("provideInlineCompletionItems: 1")
		...
		try {
			...
			console.log("provideInlineCompletionItems: 2, this.apiHandler")
			console.log(this.apiHandler)

			const model = this.apiHandler.getModel()
			console.log("before HelperVars.create", model.id, model)
			const helper = await HelperVars.create(input, options, model.id, this.ide)
			...
			const outcome: AutocompleteOutcome = {
				...
				suffix,
				prompt,
				modelProvider: this.apiHandler.getModel().id,
				modelName: model.id,
				completionOptions,
				cacheHit,
