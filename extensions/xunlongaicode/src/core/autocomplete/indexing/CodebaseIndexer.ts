...
import { CodebaseIndex, IndexResultType, PathAndCacheKey, RefreshIndexResults } from "./types.js"
...
export class CodebaseIndexer {
	...
	protected async getIndexesToBuild(): Promise<CodebaseIndex[]> {
		...
		indexes.push(new FullTextSearchCodebaseIndex(), new CodeSnippetsCodebaseIndex(this.ide))

		return indexes
	}

	private totalIndexOps(results: RefreshIndexResults): number {
		return results.compute.length + results.del.length + results.addTag.length + results.removeTag.length
	}
	...
	public async refreshFile(file: string, workspaceDirs: string[]): Promise<void> {
		...
		for (const index of indexesToBuild) {
			...
			const [fullResults, fullLastUpdated, markComplete] = await getComputeDeleteAddRemove(
				tag,
				{ ...stats },
	...
	async *refreshDirs(dirs: string[], abortSignal: AbortSignal): AsyncGenerator<IndexingProgressUpdate> {
		let progress = 0
	 ...
	 */
	private *batchRefreshIndexResults(results: RefreshIndexResults): Generator<RefreshIndexResults> {
		let curPos = 0
		while (
