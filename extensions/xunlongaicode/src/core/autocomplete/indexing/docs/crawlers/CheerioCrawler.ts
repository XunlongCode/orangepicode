...
export default class CheerioCrawler {
	...
	async *crawl(): AsyncGenerator<PageData> {
		...
		console.log(
			`[${(this.constructor as any).name}] Starting crawl from: ${this.startUrl} - Max Depth: ${this.maxDepth}`,
		)
	...
	private async getLinksFromUrl(url: string, path: string) {
		...
		try {
			...
			if (error instanceof Error && error.message.includes("maximum redirect")) {
				console.error(
					`[${(this.constructor as any).name}] Maximum redirect reached for: ${location.toString()}`,
				)
				return { html: "", links: [] }
			}
			console.error(error)
		...
		links = [...new Set(links)].filter((link) => {
			return !link.includes("#") && !this.IGNORE_PATHS_ENDING_IN.some((ending) => link.endsWith(ending))
		})
		return { html, links }
	}
