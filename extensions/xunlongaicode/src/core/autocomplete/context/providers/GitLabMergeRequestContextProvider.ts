...
const getSubprocess = async (extras: ContextProviderExtras) => {
	...
	return (command: string) => extras.ide.subprocess(command, workingDir).then(trimFirstElement)
}
...
class GitLabMergeRequestContextProvider extends BaseContextProvider {
	...
	private async getRemoteBranchInfo(extras: ContextProviderExtras): Promise<RemoteBranchInfo> {
		...
		const currentBranchInfo = branchInfo.split("\n").find((line) => line.startsWith("*"))
	...
	async getContextItems(query: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
		...
		try {
			const mergeRequests = await api
				.get<Array<GitLabMergeRequest>>(`/projects/${encodeURIComponent(project!)}/merge_requests`, {
					params: {
			...
			for (const mergeRequest of mergeRequests) {
				...
				if (extras.selectedCode.length && this.options.filterComments) {
					const toRemove = Object.keys(locations).filter(
						(filename) =>
							!extras.selectedCode.find((selection) => selection.filepath.endsWith(filename)) &&
							filename !== "general",
					)
