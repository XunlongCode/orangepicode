...
import { FilePathAndName, PackageDetails, PackageFilePathAndName, ParsedPackageInfo } from "../../../.."
...
export class PythonPackageCrawler implements PackageCrawler {
	...
	getPackageFiles(files: FilePathAndName[]): PackageFilePathAndName[] {
		...
		return files
			.filter((file) => file.name === "requirements.txt" || file.name === "Pipfile")
			.map((file) => ({
				...file,
				packageRegistry: "pypi",
			}))
	...
	parsePackageFile(file: PackageFilePathAndName, contents: string): ParsedPackageInfo[] {
	...
	async getPackageDetails(packageInfo: ParsedPackageInfo): Promise<PackageDetails> {
		...
		return {
			docsLink: (data?.info?.project_urls?.Documentation as string | undefined) ?? homePage,
			title: data?.info?.name as string | undefined,
			description: data?.info?.summary as string | undefined,
			repo: (data?.info?.project_urls?.Repository as string | undefined) ?? homePage,
