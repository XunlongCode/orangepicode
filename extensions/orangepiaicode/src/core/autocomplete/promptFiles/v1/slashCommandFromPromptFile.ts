...
async function renderPromptV1(prompt: string, context: ContinueSDK, userInput: string) {
	...
	return renderTemplatedString(prompt, context.ide.readFile.bind(context.ide), inputData, helpers)
}

export function slashCommandFromPromptFileV1(path: string, content: string): SlashCommand | null {
	const { name, description, systemMessage, prompt, version } = parsePromptFileV1V2(path, content)
	...
	return {
		...
		run: async function* (context) {
			...
			const messages = updateChatHistory(context.history, name, renderedPrompt, systemMessage)
