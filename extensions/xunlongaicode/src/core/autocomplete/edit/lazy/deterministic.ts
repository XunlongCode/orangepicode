...
function reconstructNewFile(oldFile: string, newFile: string, lazyBlockReplacements: AstReplacements): string {
	// Sort acc by reverse line number
	lazyBlockReplacements.sort((a, b) => a.nodeToReplace.startIndex - b.nodeToReplace.startIndex).reverse()
	...
	for (const { nodeToReplace: lazyBlockNode, replacementNodes } of lazyBlockReplacements) {
		// Get the full string from the replacement nodes
		let replacementText = ""
		if (replacementNodes.length > 0) {
			const startPosition = replacementNodes[0].startPosition
			const endPosition = replacementNodes[replacementNodes.length - 1].endPosition
			const replacementLines = oldFileLines.slice(startPosition.row, endPosition.row + 1)
			replacementLines[0] = replacementLines[0].slice(startPosition.column)
			replacementLines[replacementLines.length - 1] = replacementLines[replacementLines.length - 1].slice(
				0,
				endPosition.column,
			)
			...
			let newLinesFound = 0
			while (startIndex > 0 && newFileChars[startIndex - 1]?.trim() === "" && newLinesFound < 2) {
				startIndex--
	...
	if (!findInAst(newTree.rootNode, isLazyBlock)) {
		...
		if (firstSimilarNode?.parent?.equals(oldTree.rootNode)) {
			...
			if (matchingNode) {
				...
				reconstructedNewFile = oldText.slice(0, startIndex) + newTree.rootNode.text + oldText.slice(endIndex)
			} else {
...
function isLazyBlock(node: Parser.SyntaxNode): boolean {
	...
	if (node.type === "jsx_expression" && node.namedChildCount === 1 && isLazyBlock(node.namedChildren[0])) {
		return true
	}
...
function stringsWithinLevDistThreshold(a: string, b: string, threshold: number) {
	const dist = distance(a, b)
	return dist / Math.min(a.length, b.length) <= threshold
}

function programNodeIsSimilar(programNode: Parser.SyntaxNode, otherNode: Parser.SyntaxNode): boolean {
...
function nodesAreSimilar(a: Parser.SyntaxNode, b: Parser.SyntaxNode): boolean {
	...
	if (a.namedChildren[0]?.text === b.namedChildren[0]?.text && a.children[1]?.text === b.children[1]?.text) {
		return true
	}
	...
	if (
		...
		a.namedChildren[0]?.children[1]?.text === b.namedChildren[0]?.children[1]?.text
	) {
