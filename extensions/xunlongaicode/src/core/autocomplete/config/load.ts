...
import { ConfigResult, ConfigValidationError, ModelRole } from "@continuedev/config-yaml"
...
import { slashCommandFromDescription, slashFromCustomCommand } from "../commands/index"
...
function loadSerializedConfig(
	...
	config.contextProviders ??=
		ideType === "vscode" ? [...defaultContextProvidersVsCode] : [...defaultContextProvidersJetBrains]
	config.slashCommands ??= ideType === "vscode" ? [...defaultSlashCommandsVscode] : [...defaultSlashCommandsJetBrains]
...
async function serializedToIntermediateConfig(initial: SerializedContinueConfig, ide: IDE): Promise<Config> {
	...
	const promptFiles = await getAllPromptFiles(ide, initial.experimental?.promptPath, true)
...
function isModelDescription(llm: ModelDescription | CustomLLM): llm is ModelDescription {
...
async function intermediateToFinalConfig(
		...
		if (isModelDescription(desc)) {
			...
			if (llm.model === "AUTODETECT") {
				try {
					...
					models.push(...(detectedModels.filter((x) => typeof x !== "undefined") as BaseLLM[]))
				} catch (e) {
	...
	if (allowFreeTrial) {
		...
		const freeTrialModels = models.filter((model) => model.providerName === "free-trial")
	...
	const codebaseContextParams =
		(
			(config.contextProviders || []).filter(isContextProviderWithParams).find((cp) => cp.name === "codebase") as
				| ContextProviderWithParams
				| undefined
		)?.params || {}
	...
	const DEFAULT_CONTEXT_PROVIDERS = [
		...
		...(!config.disableIndexing ? [new CodebaseContextProvider(codebaseContextParams)] : []),
		// Add prompt files provider if enabled
		...(loadPromptFiles ? [new PromptFilesContextProvider({})] : []),
	]

	const DEFAULT_CONTEXT_PROVIDERS_TITLES = DEFAULT_CONTEXT_PROVIDERS.map(({ description: { title } }) => title)
	...
	for (const provider of config.contextProviders || []) {
		if (isContextProviderWithParams(provider)) {
			...
			if (instance.description.title === "continue-proxy") {
				;(instance as ContinueProxyContextProvider).workOsAccessToken = workOsAccessToken
			}
	...
	// Embeddings Provider
	function getEmbeddingsILLM(embedConfig: EmbeddingsProviderDescription | ILLM | undefined): ILLM | null {
		if (!embedConfig) {
			return null
		}
		...
		if (embeddingsProviderClass) {
			if (embeddingsProviderClass.name === "_TransformersJsEmbeddingsProvider") {
				...
				return new embeddingsProviderClass(llmOptions, (url: string | URL, init: any) =>
	...
	// Reranker
	function getRerankingILLM(rerankingConfig: ILLM | RerankerDescription | undefined): ILLM | null {
		if (!rerankingConfig) {
			return null
		}
	...
	if (config.experimental?.modelContextProtocolServers) {
		await Promise.all(
			config.experimental.modelContextProtocolServers?.map(async (server, index) => {
	...
	if (inlineEditModel) {
		const match = continueConfig.models.find((m) => m.title === inlineEditModel)
		if (match) {
	...
	const applyBlockModel = getModelByRole(continueConfig, "applyCodeBlock")?.title
	if (applyBlockModel) {
		const match = continueConfig.models.find((m) => m.title === applyBlockModel)
		if (match) {
...
async function finalToBrowserConfig(final: ContinueConfig, ide: IDE): Promise<BrowserSerializedContinueConfig> {
	return {
		...
		modelsByRole: Object.fromEntries(
			Object.entries(final.modelsByRole).map(([k, v]) => [k, v.map(llmToSerializedModelDescription)]),
...
async function downloadAndInstallEsbuild(ide: IDE) {
	...
	try {
		...
		await ide.showToast("info", `'esbuild' successfully installed to ${esbuildPath}`)
	} catch (error) {
...
async function loadContinueConfigFromJson(
	...
	} = loadSerializedConfig(workspaceConfigs, ideSettings, ideInfo.ideType, overrideConfigJson, ide)
	...
	if (ideSettings.remoteConfigServerUrl) {
		try {
			const configJsPathForRemote = getConfigJsPathForRemote(ideSettings.remoteConfigServerUrl)
