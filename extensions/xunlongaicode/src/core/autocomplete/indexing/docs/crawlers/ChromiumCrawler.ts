...
import { editConfigJson, getChromiumPath, getContinueUtilsPath } from "../../../util/paths"
...
export class ChromiumCrawler {
	...
	async *crawl(): AsyncGenerator<PageData> {
		console.debug(`[${(this.constructor as any).name}] Crawling site: ${this.startUrl}`)
	...
	private async gotoPageAndHandleRedirects(page: Page, url: string) {
		...
		const responseHandler: Handler<HTTPResponse> = (event) => (responseEventOccurred = true)
	...
	private async *crawlSitePages(
		...
		for (const linkGroup of linkGroups) {
			...
			for (const link of linkGroup) {
				...
				if (enqueuedLinkCount <= this.maxRequestsPerCrawl && depth <= this.maxDepth) {
	...
	private async getLinksFromPage(page: Page, curUrl: URL) {
		...
		const cleanedLinks = links
			...
			.filter((newUrl) => newUrl !== null && this.isValidHostAndPath(newUrl, curUrl) && newUrl !== curUrl)
...
export class ChromiumInstaller {
	...
	shouldInstallOnStartup() {
		return !this.isInstalled() && this.config.experimental?.useChromiumForDocsCrawling
	}

	shouldProposeUseChromiumOnCrawlFailure() {
		const { experimental } = this.config

		return experimental?.useChromiumForDocsCrawling === undefined || experimental.useChromiumForDocsCrawling
	}
