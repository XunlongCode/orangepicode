...
export async function getAllSuggestedDocs(ide: IDE) {
	...
	const uniqueFilePaths = Array.from(
		new Set(Object.values(packageFilesByRegistry).flatMap((files) => files.map((file) => file.path))),
	)
	...
	const fileContents = new Map(fileContentsArray.map(({ path, contents }) => [path, contents]))
	...
	registries.forEach((registry) => {
		...
		const uniquePackages = Array.from(new Map(packages.map((pkg) => [pkg.name, pkg])).values())
