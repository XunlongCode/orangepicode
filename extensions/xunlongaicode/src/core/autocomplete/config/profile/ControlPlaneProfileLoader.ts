...
export default class ControlPlaneProfileLoader implements IProfileLoader {
	...
	constructor(
		...
		setInterval(async () => {
			this.workspaceSettings = await this.controlPlaneClient.getSettingsForWorkspace(this.description.id)
			this.onReload()
	...
	async doLoadConfig(): Promise<ConfigResult<ContinueConfig>> {
		const settings =
			...
			((await this.controlPlaneClient.getSettingsForWorkspace(this.description.id)) as any)
