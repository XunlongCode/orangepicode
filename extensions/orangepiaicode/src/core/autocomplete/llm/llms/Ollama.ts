...
class Ollama extends BaseLLM implements ModelInstaller {
	...
	private _getModelFileParams(options: CompletionOptions): OllamaModelFileParams {
		return {
	...
	private _getGenerateOptions(options: CompletionOptions, prompt: string, suffix?: string): OllamaRawOptions {
		return {
