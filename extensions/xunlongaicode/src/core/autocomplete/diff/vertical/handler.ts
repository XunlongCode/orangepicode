...
import type { ApplyState, DiffLine } from "../.."
...
export class VerticalDiffHandler implements vscode.Disposable {
	...
	async clear(accept: boolean) {
		...
		const rangesToDelete = accept ? this.redDecorationManager.getRanges() : this.greenDecorationManager.getRanges()
	...
	async acceptRejectBlock(
		...
		if (numRed > 0) {
			const rangeToDelete = this.redDecorationManager.deleteRangeStartingAt(startLine)
