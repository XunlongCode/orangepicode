...
const SUPPORTED_MODEL_TITLE_FAMILIES = ["claude-3", "llama3.1", "llama3.2", "gemini-1.5", "gpt-4"]

function isSupportedModel(config: ContinueConfig, modelTitle?: string): boolean {
	...
	return SUPPORTED_MODEL_TITLE_FAMILIES.some((title) => lowercaseModelTitle.includes(title))
}
