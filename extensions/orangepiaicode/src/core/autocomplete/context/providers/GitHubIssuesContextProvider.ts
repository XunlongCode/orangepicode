...
class GitHubIssuesContextProvider extends BaseContextProvider {
	...
	async getContextItems(query: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
		...
		const octokit = new Octokit({
			...
			baseUrl: this.options?.domain ? `https://${this.options.domain}/api/v3` : undefined,
			request: {
				fetch: extras.fetch,
			},
		})
		...
		const parts = [issue.data.body || "No description", ...comments.data.map((comment) => comment.body)]
		content += `\n\n${parts.join("\n\n---\n\n")}`
