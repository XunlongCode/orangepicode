...
async function resolveAttachment(name: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
	// Context providers
	const contextProvider = extras.config.contextProviders?.find((provider) => provider.description.title === name)
	if (contextProvider) {
	...
	if (resolvedFileUri) {
		...
		if (name.endsWith(".prompt")) {
			// Recurse
			const [items, _] = await renderPromptFileV2(await extras.ide.readFile(name), extras)
			subItems.push(...items)
		}
	...
	if (name.startsWith("http")) {
		const items = await new URLContextProvider({}).getContextItems(name, extras)
		return items
	}
