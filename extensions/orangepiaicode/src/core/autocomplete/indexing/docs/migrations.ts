...
export async function runSqliteMigrations(db: Database) {
	await new Promise((resolve) => {
		void migrate(
			...
			async () => {
				try {
					const pragma = await db.all(`PRAGMA table_info(${DocsService.sqlitebTableName});`)

					const hasFaviconCol = pragma.some((pragma) => pragma.name === "favicon")
					if (!hasFaviconCol) {
						await db.exec(`ALTER TABLE ${DocsService.sqlitebTableName} ADD COLUMN favicon BLOB;`)
					}

					const hasBaseUrlCol = pragma.some((pragma) => pragma.name === "baseUrl")
					if (hasBaseUrlCol) {
						await db.exec(`ALTER TABLE ${DocsService.sqlitebTableName} RENAME COLUMN baseUrl TO startUrl;`)
					}

					const needsToUpdateConfig = !hasFaviconCol || hasBaseUrlCol
					if (needsToUpdateConfig) {
						const sqliteDocs = await db.all<Array<Pick<SqliteDocsRow, "title" | "startUrl">>>(
