...
export async function getContextProviderItems({ contextProviders }: ContinueConfig): Promise<QuickPickItem[]> {
	if (!contextProviders) {
		return []
	}
		...
		await Promise.all(
			selectedProviders?.map((selectedProvider) => {
				const provider = config.contextProviders?.find(
					(provider) => provider.description.displayTitle === selectedProvider.label,
				)
...
export async function getContextProviderQuickPickVal(config: ContinueConfig, ide: IDE) {
