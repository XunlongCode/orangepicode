	...
	async getContextItems(query: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
		...
		const items = await Promise.all(
			problems.map(async (problem) => {
				const { relativePathOrBasename, baseName } = getUriDescription(problem.filepath, workspaceDirs)
				const content = await ide.readFile(problem.filepath)
				const lines = content.split("\n")
				const rangeContent = lines
					.slice(Math.max(0, problem.range.start.line - 2), problem.range.end.line + 2)
					.join("\n")
