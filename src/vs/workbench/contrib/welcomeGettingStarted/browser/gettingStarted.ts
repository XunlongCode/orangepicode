...
export class GettingStartedPage extends EditorPane {
	...
	private async buildAnnouncementList(): Promise<GettingStartedIndexList<AnnouncementEntry>> {
		...
		const announcementList = this.announcementList = new GettingStartedIndexList({
			title: localize('announcements', "OrangePi Code Announcements"),
			klass: 'announcements',
			limit: 5,
		...
		if (!this.announcementData) {
			...
			if (showExtras) {
				...
				await fetch(`https://raw.githubusercontent.com/XunlongCode/orangepicode/${branch}/announcements-extra.json`)
					.then(async res => {
						if (res.ok) {
