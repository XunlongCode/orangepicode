...
import { AssistantUnrolled, ConfigResult, FQSN, FullSlug, SecretResult } from "@continuedev/config-yaml"
...
export class ControlPlaneClient {
	constructor(
		private readonly sessionInfoPromise: Promise<ControlPlaneSessionInfo | undefined>,
		private readonly ideSettingsPromise: Promise<IdeSettings>,
	) {}

	async resolveFQSNs(fqsns: FQSN[], orgScopeId: string | null): Promise<(SecretResult | undefined)[]> {
	...
	get userId(): Promise<string | undefined> {
		return this.sessionInfoPromise.then((sessionInfo) => sessionInfo?.account.id)
	}
	...
	> {
		...
		try {
			const url = organizationId ? `ide/list-assistants?organizationId=${organizationId}` : "ide/list-assistants"
	...
	public async listAssistantFullSlugs(organizationId: string | null): Promise<FullSlug[] | null> {
		const userId = await this.userId
		if (!userId) {
			return null
		}
