...
class WatsonX extends BaseLLM {
	...
	protected async *_streamChat(
		...
		if (watsonxToken === undefined || now > watsonxToken.expiration || watsonxToken.token === undefined) {
		...
		const stopSequences = options.stop?.slice(0, 6) ?? (options.model?.includes("granite") ? ["Question:"] : [])
