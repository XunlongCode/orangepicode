...
class VertexAI extends BaseLLM {
	...
	private static getDefaultApiBaseFrom(options: LLMOptions) {
		...
		if (!region || !projectId) {
			throw new Error("region and projectId must be defined if apiBase is not provided")
		}
	...
	protected async *StreamChatAnthropic(
		...
		const systemMessage: string = renderChatMessage(messages.filter((m) => m.role === "system")[0])
	...
	private async *streamChatGemini(messages: ChatMessage[], options: CompletionOptions): AsyncGenerator<ChatMessage> {
	...
	private async *streamChatBison(messages: ChatMessage[], options: CompletionOptions): AsyncGenerator<ChatMessage> {
	...
	protected async *_streamChat(
		...
		const convertedMsgs = isV1API ? this.geminiInstance.removeSystemMessage(messages) : messages
