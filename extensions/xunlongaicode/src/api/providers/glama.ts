				...
				headers: {
					"X-Glama-Metadata": JSON.stringify({
						labels: [
							{
								key: "app",
								value: "vscode.xunlongcode.orangepiaicode",
							},
						],
					}),
				},
	...
	protected async *_createComplete(
		...
		try {
			...
			const response = await this.client.completions.create(
				{
					...
					temperature:
						completionOptions?.temperature ?? this.options.modelTemperature ?? GLAMA_DEFAULT_TEMPERATURE,
			...
			if ("error" in response) {
				...
				throw new Error(`Glama API Error ${error?.code}: ${error?.message}`)
			}
