...
class CustomLLMClass extends BaseLLM {
	...
	protected async *_streamChat(
		...
		if (this.customStreamChat) {
			for await (const content of this.customStreamChat(messages, signal, options, (...args) =>
				this.fetch(...args),
			)) {
	...
	protected async *_streamComplete(
		...
		if (this.customStreamCompletion) {
			for await (const content of this.customStreamCompletion(prompt, signal, options, (...args) =>
				this.fetch(...args),
			)) {
				yield content
			}
