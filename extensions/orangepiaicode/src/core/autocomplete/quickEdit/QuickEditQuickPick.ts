...
export class QuickEdit {
	 ...
	 */
	private static hasFileSearchQueryRegex = new RegExp(`${FILE_SEARCH_CHAR}[^${FILE_SEARCH_CHAR}\\s]+(?!\\s)$`)
	...
	async show(params?: QuickEditShowParams) {
		...
		const hasChanges = !!this.verticalDiffManager.getHandlerForFile(editor.document.uri.toString())
	...
	private async initiateNewQuickPick(editor: vscode.TextEditor, params: QuickEditShowParams | undefined) {
		this.clear()
		...
		const { label: selectedLabel, value: selectedValue } = await this._getInitialQuickPickVal()
	...
	private openAcceptRejectMenu(prompt: string, path: string | undefined) {
		...
		quickPick.placeholder = "Accept or reject changes. Start typing to try again with a new prompt."
		quickPick.show()

		quickPick.onDidChangeValue(() => {
			quickPick.items = [prompt, ""].includes(quickPick.value) ? REVIEW_CHANGES_ITEMS : [SUBMIT_ITEM]
		})
	...
	private handleUserPrompt = async (prompt: string, path: string | undefined) => {
	...
	private setActiveEditorAndPrevInput(editor: vscode.TextEditor) {
		const existingHandler = this.verticalDiffManager.getHandlerForFile(editor.document.uri.toString())
	...
	private async getCurModelTitle(): Promise<string | undefined> {
		...
		return (
			...
			(await this.webviewProtocol.request("getDefaultModelTitle", undefined, false)) ??
	...
	private async _streamEditWithInputAndContext(prompt: string, modelTitle: string) {
		...
		for (const fileRef of fileReferences) {
			...
			prompt = prompt.replace(fileRef, `\`\`\`${filePath}\n${fileContent}\n\`\`\`\n\n`)
		}
	...
	async _getInitialQuickPickVal(): Promise<{
		...
		quickPick.placeholder = "Enter a prompt to edit your code (@ to search files, ⏎ to submit)"
		...
		quickPick.onDidChangeValue((value) => this.handleQuickPickChange({ value, quickPick, initialItems }))
		/**
		 ...
		 */
		const selectedItemLabel = await new Promise<QuickEditInitialItemLabels | undefined>((resolve) =>
	...
	private handleQuickPickAccept = ({
		...
		const isFileSelection = !Object.values(QuickEditInitialItemLabels).includes(label as QuickEditInitialItemLabels)

		if (isFileSelection) {
			// Replace the file search query with the selected file path
			const curValue = quickPick.value
			const newValue = curValue.substring(0, curValue.lastIndexOf("@") + 1) + label + " "
	...
	private async handleSelect({
		...
		switch (selectedLabel) {
			...
			case QuickEditInitialItemLabels.ContextProviders:
				const contextProviderVal = await getContextProviderQuickPickVal(config, this.ide)
