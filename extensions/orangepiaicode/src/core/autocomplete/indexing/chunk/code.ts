...
function firstChild(node: SyntaxNode, grammarName: string | string[]): SyntaxNode | null {
...
async function collapseChildren(
	...
	if (block) {
		...
		for (const child of childrenToCollapse.reverse()) {
			...
			if (grandChild) {
				...
				code = code.slice(0, start) + collapsedReplacement(grandChild) + code.slice(end)
	...
	while ((await countTokensAsync(code.trim())) > maxChunkSize && collapsedChildren.length > 0) {
		removedChild = true
...
async function constructClassDefinitionChunk(node: SyntaxNode, code: string, maxChunkSize: number): Promise<string> {
...
async function constructFunctionDefinitionChunk(node: SyntaxNode, code: string, maxChunkSize: number): Promise<string> {
	const bodyNode = node.children[node.children.length - 1]
	const funcText = code.slice(node.startIndex, bodyNode.startIndex) + collapsedReplacement(bodyNode)
