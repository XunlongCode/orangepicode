...
export function getTheme() {
	...
	const autoDetectColorScheme = workbenchConfig.get<boolean>("window.autoDetectColorScheme")
	const autoDetectHighContrast = workbenchConfig.get<boolean>("window.autoDetectHighContrast")
	...
	try {
					...
					if (theme.id === colorTheme || theme.label === colorTheme) {
						...
						while (currentParsedTheme.include) {
							...
							if (fs.existsSync(includeThemePath)) {
								const includeThemeString = fs.readFileSync(includeThemePath).toString()
		...
		let convertedTheme: { base: string; include?: string } & Record<string, any> = convertTheme(parsed)
