...
export async function retrieveContextItemsFromEmbeddings(
	...
	const nFinal = options?.nFinal ?? Math.min(DEFAULT_N_FINAL, contextLength / tokensPerSnippet / 2)
	...
	const pipelineType = useReranking ? RerankerRetrievalPipeline : NoRerankerRetrievalPipeline
	...
	return [
		...
		...results
			...
			.map((r) => {
				const { relativePathOrBasename, last2Parts, baseName } = getUriDescription(r.filepath, workspaceDirs)
