...
export function rerenderDecorations(editorUri: string) {
	...
	const rangesWithoutEmptyLastLine = (ranges: vscode.Range[]) => {
		...
		for (let i = 0; i < ranges.length; i++) {
			...
			if (range.start.line === range.end.line && range.start.character === 0 && range.end.character === 0) {
...
function selectSuggestion(accept: SuggestionSelectionOption, key: SuggestionRanges | null = null) {
	const editor = vscode.window.activeTextEditor
	if (!editor) {
		return
	}
	...
	if (key) {
		...
		for (let i = 0; i < suggestions.length; i++) {
			if (suggestions[i].newRange === key.newRange && suggestions[i].oldRange === key.oldRange) {
	...
	switch (accept) {
		...
		case "new":
			...
			break
		case "selected":
			rangeToDelete = suggestion.newSelected ? suggestion.oldRange : suggestion.newRange
	}
	...
	for (const below of suggestions) {
		...
		if (
			below.oldRange.union(below.newRange).start.line > suggestion.oldRange.union(suggestion.newRange).start.line
		) {
...
export async function rejectSuggestionCommand(key: SuggestionRanges | null = null) {
	selectSuggestion("old", key)
}

export async function showSuggestion(editorUri: vscode.Uri, range: vscode.Range, suggestion: string): Promise<boolean> {
	// Check for empty suggestions:
	if (suggestion === "" && range.start.line === range.end.line && range.start.character === range.end.character) {
