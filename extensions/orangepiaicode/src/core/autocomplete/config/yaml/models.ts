...
function getModelClass(model: ModelConfig): (typeof LLMClasses)[number] | undefined {
	return LLMClasses.find((llm) => llm.providerName === model.provider)
}

function getContinueProxyModelName(ownerSlug: string, packageSlug: string, model: ModelConfig): string {
...
async function modelConfigToBaseLLM(
	...
	// 从 model 对象中提取 capabilities 属性，并创建一个不包含该属性的新对象
	const { capabilities, ...modelWithoutCapabilities } = model
	...
	// 将 model.capabilities 转换为 ModelCapability 类型
	if (capabilities) {
	...
	const env = model.env ?? {}
	if ("useLegacyCompletionsEndpoint" in env && typeof env.useLegacyCompletionsEndpoint === "boolean") {
