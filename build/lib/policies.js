...
class BasePolicy {
    ...
    renderADMX(regKey) {
        return [
            `<policy name="${this.name}" class="Both" displayName="$(string.${this.name})" explainText="$(string.${this.name}_${this.description.nlsKey.replace(/\./g, '_')})" key="Software\\Policies\\XunlongCode\\${regKey}" presentation="$(presentation.${this.name})">`,
...
<policyDefinitions revision="1.1" schemaVersion="1.0">
	<policyNamespaces>
		<target prefix="${regKey}" namespace="XunlongCode.Policies.${regKey}" />
	</policyNamespaces>
