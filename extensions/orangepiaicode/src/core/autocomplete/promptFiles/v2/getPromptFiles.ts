...
export async function getPromptFilesFromDir(ide: IDE, dir: string): Promise<{ path: string; content: string }[]> {
	try {
...
export async function getAllPromptFiles(
	...
	const fullDirs = workspaceDirs.map((dir) => dirsToCheck.map((d) => joinPathsToUri(dir, d))).flat()
