...
class VsCodeIde implements IDE {
					...
					.then(async (selection) => {
						if (selection === "Use API key / local model") {
							...
							editConfigJson((config) => {
								...
								} else if (config.tabAutocompleteModel?.provider === "free-trial") {
							...
							vscode.env.openExternal(
								vscode.Uri.parse("https://docs.continue.dev/reference/Model%20Providers/freetrial"),
							)
					...
					.then((selection) => {
						if (selection === "Remove for me") {
							editConfigJson((configJson) => {
								configJson.models = configJson.models.filter((model) => model.provider !== "free-trial")
	...
	getIdeInfo(): Promise<IdeInfo> {
		return Promise.resolve({
			...
			extensionVersion: vscode.extensions.getExtension("continue.continue")?.packageJSON.version,
		})
	}
	...
	async getWorkspaceConfigs() {
		const workspaceDirs = vscode.workspace.workspaceFolders?.map((folder) => folder.uri) || []
		...
		for (const workspaceDir of workspaceDirs) {
			...
			for (const [filename, type] of files) {
				if (
					...
					const contents = await this.readFile(vscode.Uri.joinPath(workspaceDir, filename).toString())
	...
	private getIdeSettingsSync(): IdeSettings {
		...
		const ideSettings: IdeSettings = {
			...
			enableControlServerBeta: settings.get<boolean>("enableContinueForTeams", false),
