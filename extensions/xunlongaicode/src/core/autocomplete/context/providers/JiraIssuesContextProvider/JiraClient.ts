...
export class JiraClient {
	...
	async issue(issueId: string, customFetch: (url: string | URL, init: any) => Promise<any>): Promise<Issue> {
		...
		const response = await customFetch(
			new URL(this.baseUrl + `/issue/${issueId}?fields=description,comment,summary`),
			{
				method: "GET",
				headers: {
		...
		result.comments =
			issue.fields.comment?.comments?.map((comment: any) => {
				const body = typeof comment.body === "string" ? comment.body : adf2md.validate(comment.body).result
	...
	async listIssues(customFetch: (url: string | URL, init: any) => Promise<any>): Promise<Array<QueryResult>> {
		const response = await customFetch(
			new URL(
				this.baseUrl +
