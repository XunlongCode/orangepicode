...
import { CodebaseIndex, IndexResultType, MarkCompleteCallback, PathAndCacheKey, RefreshIndexResults } from "./types"
...
export class LanceDbIndex implements CodebaseIndex {
	...
	static async create(
		...
		try {
			...
			return new LanceDbIndex(embeddingsProvider, readFile, continueServerClient)
		} catch (err) {
	...
	private async getChunks(item: PathAndCacheKey, content: string): Promise<Chunk[]> {
	...
	private createLanceDbRows(chunkMap: ChunkMap, embeddings: number[][]): LanceDbRow[] {
	...
	async *update(
		...
		const addComputedLanceDbRows = async (pathAndCacheKeys: PathAndCacheKey[], computedRows: LanceDbRow[]) => {
			if (lanceTable) {
		...
		if (this.continueServerClient?.connected) {
			try {
				...
				const resp = await this.continueServerClient.getFromIndexCache(keys, "embeddings", repoName)
				for (const [cacheKey, chunks] of Object.entries(resp.files)) {
					const path = results.compute.find((item) => item.cacheKey === cacheKey)?.path
					if (!path) {
						console.warn("Continue server sent a cacheKey that wasn't requested", cacheKey)
						continue
					}
	...
	async retrieve(
		...
		allResults = allResults.sort((a, b) => a._distance - b._distance).slice(0, n)

		const sqliteDb = await SqliteDb.get()
		const data = await sqliteDb.all(
			`SELECT * FROM lance_db_cache WHERE uuid in (${allResults.map((r) => `'${r.uuid}'`).join(",")})`,
		)
		...
		return new Promise<void>((resolve, reject) => {
			db.db.serialize(() => {
				...
				db.db.exec("COMMIT", (err: Error | null) => {
					if (err) {
						reject(
							new Error("error while committing insert into lance_db_rows transaction", { cause: err }),
						)
					} else {
						resolve()
					}
