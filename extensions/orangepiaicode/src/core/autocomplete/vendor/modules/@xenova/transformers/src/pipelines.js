	...
	extends /** @type {new (options: TextPipelineConstructorArgs) => FeatureExtractionPipelineType} */ (Pipeline)
{
	...
	async _call(texts, { pooling = /** @type {'none'} */ ("none"), normalize = false } = {}) {
		// Run tokenization
		const model_inputs = this.tokenizer(texts, {
			padding: true,
			truncation: true,
	...
	)
{
	...
	async _call(images, candidate_labels, { hypothesis_template = "This is a photo of {}" } = {}) {
		...
		const function_to_apply =
			this.model.config.model_type === "siglip" ? (batch) => batch.sigmoid().data : (batch) => softmax(batch.data)
	...
	)
{
	...
	async _call(images, candidate_labels, { threshold = 0.1, topk = null, percentage = false } = {}) {
		const isBatched = Array.isArray(images)
		const preparedImages = await prepareImages(images)
		...
		for (let i = 0; i < preparedImages.length; ++i) {
			...
			// @ts-ignore
			const processed = this.processor.feature_extractor.post_process_object_detection(
				output,
				threshold,
				imageSize,
				true,
