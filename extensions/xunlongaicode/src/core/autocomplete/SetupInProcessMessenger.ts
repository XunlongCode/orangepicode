...
import { FromCoreProtocol, FromWebviewProtocol, ToCoreProtocol } from "./protocol"
import { ToWebviewFromCoreProtocol } from "./protocol/coreWebview"
import { ToIdeFromWebviewOrCoreProtocol } from "./protocol/ide"
import { ToIdeFromCoreProtocol } from "./protocol/ideCore"
import { InProcessMessenger, Message } from "./protocol/messenger"
import { CORE_TO_WEBVIEW_PASS_THROUGH, WEBVIEW_TO_CORE_PASS_THROUGH } from "./protocol/passThrough"
...
import { applyCodeBlock } from "./edit/lazy/applyCodeBlock"
	...
	constructor(
		...
		this.onWebview("applyToFile", async ({ data }) => {
			...
			if (!llm) {
				...
				if (!llm) {
					vscode.window.showErrorMessage(`Model ${data.curSelectedModelTitle} not found in config.`)
					return
				}
			}
			...
			if (instant) {
				await verticalDiffManager.streamDiffLines(diffLines, instant, data.streamId)
			} else {
				...
				const rangeToApplyTo = editor.selection.isEmpty ? fullEditorRange : editor.selection
		...
		WEBVIEW_TO_CORE_PASS_THROUGH.forEach((messageType) => {
			...
			this.onWebview(messageType, async (msg) => {
				console.log("webview to core", messageType, msg)
				return await this.inProcessMessenger.externalRequest(messageType, msg.data, msg.messageId)
			})
		})
		...
		this.onWebviewOrCore("getGitHubAuthToken", (msg) => ide.getGitHubAuthToken(msg.data))
