...
export function rifWithContentsToContextItem(rif: RangeInFileWithContents): ContextItemWithId {
	const basename = getUriPathBasename(rif.filepath)
	const { relativePathOrBasename, foundInDir, uri } = findUriInDirs(rif.filepath, window.workspacePaths ?? [])
...
export function ctxItemToRifWithContents(item: ContextItemWithId, linesOffByOne = false): RangeInFileWithContents {
	let startLine = 0
	let endLine = 0
