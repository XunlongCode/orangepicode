...
import { orangePiCodeAuxiliaryBarAllowedViewContainerIDs } from './shared.js';

export class OrangePiCodeViewDescriptorService extends ViewDescriptorService implements IViewDescriptorService {
	constructor(
	...
	override moveViewContainerToLocation(
		...
		const isAllowed = () => {
			return orangePiCodeAuxiliaryBarAllowedViewContainerIDs.some(id => viewContainer.id.includes(id));
		};
		...
		// Prevent integrations from moving out of AuxiliaryBar
		if (location !== ViewContainerLocation.AuxiliaryBar && isAllowed()) {
			return;
		}
