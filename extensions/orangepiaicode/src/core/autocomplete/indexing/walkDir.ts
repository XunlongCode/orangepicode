...
import { defaultIgnoreFileAndDir, getGlobalContinueIgArray, gitIgArrayFromFile } from "./ignore"
...
class DFSWalker {
	...
	public async *walk(): AsyncGenerator<string> {
		const defaultAndGlobalIgnores = ignore().add(defaultIgnoreFileAndDir).add(getGlobalContinueIgArray())
		...
		for (let cur = stack.pop(); cur; cur = stack.pop()) {
			...
			const newIgnore = await getIgnoreContext(cur.walkableEntry.uri, entries, this.ide, defaultAndGlobalIgnores)
...
export async function walkDir(uri: string, ide: IDE, _optionOverrides?: WalkerOptions): Promise<string[]> {
...
export async function* walkDirAsync(path: string, ide: IDE, _optionOverrides?: WalkerOptions): AsyncGenerator<string> {
