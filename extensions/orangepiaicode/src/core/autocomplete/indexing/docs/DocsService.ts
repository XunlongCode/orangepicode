...
import { addContextProvider, isSupportedLanceDbCpuTargetForLinux } from "../../config/util"
...
import { editConfigJson, getDocsSqlitePath, getLanceDbPath } from "../../util/paths"
import { Telemetry } from "../../util/posthog"

import { ArticleWithChunks, htmlPageToArticleWithChunks, markdownPageToArticleWithChunks } from "./article"
...
import { downloadFromS3, getS3Filename, S3Buckets, SiteIndexingResults } from "./preIndexed"
...
export default class DocsService {
	...
	static preIndexedDocsEmbeddingsProvider = new TransformersJsEmbeddingsProvider()
	...
	async initStatuses(): Promise<void> {
		...
		this.config.docs?.forEach((doc) => {
			...
			const sharedStatus: Omit<IndexingStatus, "progress" | "description" | "status"> = {
				type: "docs",
				id: doc.startUrl,
				isReindexing: false,
				title: doc.title,
			...
			if (this.config.selectedModelByRole.embed) {
				sharedStatus.embeddingsProviderId = this.config.selectedModelByRole.embed.embeddingId
			}
			const indexedStatus: IndexingStatus = metadata.find((meta) => meta.startUrl === doc.startUrl)
				? {
						...sharedStatus,
						progress: 0,
						description: "Pending",
	...
	private async handleConfigUpdate({ config: newConfig }: ConfigResult<ContinueConfig>) {
		if (newConfig) {
	...
	async syncDocsWithPrompt(reIndex: boolean = false) {
		if (!this.hasDocsContextProvider()) {
			...
			const res = await this.ide.showToast("info", "Starting docs indexing", actionMsg)
			...
			if (res === actionMsg) {
				...
				void this.ide.showToast("info", "Successfuly added docs context provider")
			} else {
				return
			}
		}
	...
	async indexAndAdd(siteIndexingConfig: SiteIndexingConfig, forceReindex: boolean = false): Promise<void> {
		...
		const fixedStatus: Omit<IndexingStatus, "progress" | "description" | "status"> = {
			type: "docs",
		...
		if (!forceReindex) {
			...
			const hasFailed = failedDocs.find((d) => this.siteIndexingConfigsAreEqual(siteIndexingConfig, d))
			if (hasFailed) {
				console.log(
		...
		const markFailedInGlobalContext = () => {
			const globalContext = new GlobalContext()
			const failedDocs = globalContext.get("failedDocs") ?? []
			const newFailedDocs = failedDocs.filter((d) => !this.siteIndexingConfigsAreEqual(siteIndexingConfig, d))
		...
		try {
			...
			while (!done) {
				...
				if (result.done) {
					...
					this.handleStatusUpdate({
						...
						status: "indexing",
						progress: 0.15 * estimatedProgress + Math.min(0.35, (0.35 * processedPages) / 500),
			...
			const articleChunker =
				usedCrawler === "github" ? markdownPageToArticleWithChunks : htmlPageToArticleWithChunks
			...
			for (let i = 0; i < articles.length; i++) {
				...
				try {
					const subpathEmbeddings =
						article.chunks.length > 0 ? await provider.embed(article.chunks.map((c) => c.content)) : []
			...
			if (e instanceof Error) {
				if (e.message.includes("github.com") && e.message.includes("rate limit")) {
	...
	private async fetchAndAddPreIndexedDocEmbeddings(title: string) {
		const data = await downloadFromS3(
			...
			getS3Filename(DocsService.preIndexedDocsEmbeddingsProvider.embeddingId, title),
		)
		...
		const favicon = typeof faviconUrl === "string" ? await getFaviconBase64(faviconUrl) : undefined
	...
	async retrieveChunksFromQuery(query: string, startUrl: string, nRetrieve: number) {
		const { isPreindexed, provider } = await this.getEmbeddingsProvider(startUrl)
	...
	async getDetails(startUrl: string): Promise<DocsIndexingDetails> {
		...
		try {
			...
			const rows = (await table.filter(`starturl = '${startUrl}'`).limit(1000).execute()) as LanceDbDocsRow[]
	...
	async retrieveChunks(
		...
		try {
			docs = await table.search(vector).limit(nRetrieve).where(`starturl = '${startUrl}'`).execute()
		} catch (e: any) {
		...
		if (docs.length === 0 && isPreindexed) {
			...
			await this.fetchAndAddPreIndexedDocEmbeddings(preIndexedDocs[startUrl]!["title"])
  ...
  */
	private async syncDocs(oldConfig: ContinueConfig | undefined, newConfig: ContinueConfig, forceReindex: boolean) {
		try {
			this.isSyncing = true
			...
			const deletedDocs = currentlyIndexedDocs.filter(
				(doc) => !preIndexedDocs[doc.startUrl] && !newConfigStartUrls.includes(doc.startUrl),
			)
			...
			for (const doc of newConfigDocs) {
				...
				if (currentIndexedDoc) {
					...
					if (oldConfigDoc && !this.siteIndexingConfigsAreEqual(oldConfigDoc, doc)) {
	...
	private hasDocsContextProvider() {
		return !!this.config.contextProviders?.some(
			(provider) => provider.description.title === DocsContextProvider.description.title,
		)
	}
	...
	private async addToLance({ chunks, siteIndexingConfig, embeddings }: AddParams) {
	...
	private async addMetadataToSqlite({ siteIndexingConfig: { title, startUrl }, favicon }: AddParams) {
		if (!this.config.selectedModelByRole.embed) {
	...
	private siteIndexingConfigsAreEqual(config1: SiteIndexingConfig, config2: SiteIndexingConfig) {
		return (
