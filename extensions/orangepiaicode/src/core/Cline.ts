...
import { IDE } from "./autocomplete"
import { ConfigHandler } from "./autocomplete/config/ConfigHandler"
...
export class Cline extends EventEmitter<ClineEvents> {
	...
	ide: IDE | null = null
	configHandler: ConfigHandler | null = null
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[OrangePi AI#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[OrangePi AI#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	async presentAssistantMessage() {
		if (this.abort) {
			throw new Error(`[OrangePi AI#presentAssistantMessage] task ${this.taskId}.${this.instanceId} aborted`)
		}
							...
							if (block.partial) {
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`OrangePi AI tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
	...
	async recursivelyMakeClineRequests(
		...
		if (this.abort) {
			throw new Error(`[OrangePi AI#recursivelyMakeClineRequests] task ${this.taskId}.${this.instanceId} aborted`)
		}
		...
		if (this.consecutiveMistakeCount >= 3) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "OrangePi AI Code uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
		...
		try {
			...
			if (this.abort || this.abandoned) {
				throw new Error(
					`[OrangePi AI#recursivelyMakeClineRequests] task ${this.taskId}.${this.instanceId} aborted`,
				)
			}
