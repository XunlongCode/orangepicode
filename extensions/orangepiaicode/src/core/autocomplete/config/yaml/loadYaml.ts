...
import { ContinueConfig, IContextProvider, IDE, IdeInfo, IdeSettings, SlashCommand } from "../.."
...
async function slashCommandsFromV1PromptFiles(ide: IDE): Promise<SlashCommand[]> {
...
async function configYamlToContinueConfig(
			...
			if (embeddingsProviderClass) {
				if (embeddingsProviderClass.name === "_TransformersJsEmbeddingsProvider") {
					...
					continueConfig.modelsByRole.embed.push(
						new embeddingsProviderClass(options, (url: string | URL, init: any) =>
	...
	if (allowFreeTrial) {
		...
		const freeTrialModels = continueConfig.models.filter((model) => model.providerName === "free-trial")
		...
		continueConfig.models = continueConfig.models.filter((model) => model.providerName !== "free-trial")
	}
	...
	if (
		...
		!continueConfig.contextProviders?.some((cp) => cp.description.title === "docs")
	) {
	...
	await Promise.all(
		config.mcpServers?.map(async (server) => {
			...
			const mcpConnectionTimeout = setTimeout(() => abortController.abort(), 5000)
...
export async function loadContinueConfigFromYaml(
	...
	const rawYaml = overrideConfigYaml === undefined ? fs.readFileSync(getConfigYamlPath(ideInfo.ideType), "utf-8") : ""
