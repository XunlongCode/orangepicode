	...
	implements IMessenger<ToProtocol, FromProtocol>
{
	...
	async request<T extends keyof FromProtocol>(messageType: T, data: FromProtocol[T][0]): Promise<FromProtocol[T][1]> {
	...
	externalOn<T extends keyof FromProtocol>(messageType: T, handler: (message: Message) => any) {
