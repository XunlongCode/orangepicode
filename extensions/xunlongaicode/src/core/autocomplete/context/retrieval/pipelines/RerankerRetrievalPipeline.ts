...
export default class RerankerRetrievalPipeline extends BaseRetrievalPipeline {
	private async _retrieveInitial(args: RetrievalPipelineRunArguments): Promise<Chunk[]> {
		...
		try {
			embeddingsChunks = !!config.selectedModelByRole.embed ? await this.retrieveEmbeddings(input, nRetrieve) : []
		} catch (error) {
		...
		try {
			recentlyEditedFilesChunks = await this.retrieveAndChunkRecentlyEditedFiles(nRetrieve)
		} catch (error) {
		...
		const deduplicatedRetrievalResults: Chunk[] = deduplicateChunks(retrievalResults)
	...
	private async _rerank(input: string, chunks: Chunk[]): Promise<Chunk[]> {
		...
		let scores: number[] = await this.options.config.selectedModelByRole.rerank.rerank(input, chunks)
		...
		results.sort((a, b) => scores[chunkIndexMap.get(a)!] - scores[chunkIndexMap.get(b)!])
		results = results.slice(-this.options.nFinal)
		return results
	}

	private async _expandWithEmbeddings(chunks: Chunk[]): Promise<Chunk[]> {
		const topResults = chunks.slice(-RETRIEVAL_PARAMS.nResultsToExpandWithEmbeddings)

		const expanded = await Promise.all(
			topResults.map(async (chunk, i) => {
				const results = await this.retrieveEmbeddings(chunk.content, RETRIEVAL_PARAMS.nEmbeddingsExpandTo)
				return results
			}),
		)
