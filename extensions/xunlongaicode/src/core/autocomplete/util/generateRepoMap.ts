...
class RepoMapGenerator {
	...
	constructor(
		...
		this.maxRepoMapTokens = llm.contextLength * this.REPO_MAX_CONTEXT_LENGTH_RATIO
	}
	...
	async generate(): Promise<string> {
		...
		if (this.options.includeSignatures) {
			...
			let offset = 0
			while (true) {
				const { groupedByUri, hasMore } = await CodeSnippetsCodebaseIndex.getPathsAndSignatures(
					this.allUris,
					offset,
					this.BATCH_SIZE,
				)
...
export default async function generateRepoMap(llm: ILLM, ide: IDE, options: RepoMapOptions): Promise<string> {
