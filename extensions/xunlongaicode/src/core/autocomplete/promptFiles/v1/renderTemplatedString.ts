...
export async function renderTemplatedString(
	...
	const helperPromises = availableHelpers ? registerHelpers(availableHelpers) : {}

	const ctxProviderNames = availableHelpers?.map((h) => h[0]) ?? []

	const [newTemplate, data] = await prepareTemplateAndData(template, readFile, inputData, ctxProviderNames)
