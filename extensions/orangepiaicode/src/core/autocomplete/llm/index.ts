...
import { BaseLlmApi, ChatCompletionCreateParams, constructLlmApi } from "@continuedev/openai-adapters"
...
export abstract class BaseLLM implements ILLM {
	...
	constructor(_options: LLMOptions) {
		...
		this.contextLength = options.contextLength ?? llmInfo?.contextLength ?? DEFAULT_CONTEXT_LENGTH
	...
	private _compileChatMessages(options: CompletionOptions, messages: ChatMessage[], functions?: any[]) {
		let contextLength = this.contextLength
		if (options.model !== this.model && options.model in CONTEXT_LENGTH_FOR_MODEL) {
			contextLength = CONTEXT_LENGTH_FOR_MODEL[options.model] || DEFAULT_CONTEXT_LENGTH
		}
	...
	private _compilePromptForLog(prompt: string, completionOptions: CompletionOptions): string {
	...
	async *streamFim(
		...
		if (logEnabled) {
			if (this.writeLog) {
				await this.writeLog(this._compilePromptForLog(fimLog, completionOptions))
			}
	...
	async *streamComplete(_prompt: string, signal: AbortSignal, options: LLMFullCompletionOptions = {}) {
		...
		if (logEnabled) {
			if (this.writeLog) {
				await this.writeLog(this._compilePromptForLog(prompt, completionOptions))
			}
			if (this.llmRequestHook) {
