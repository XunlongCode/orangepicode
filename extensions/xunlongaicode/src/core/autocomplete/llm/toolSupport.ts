export const PROVIDER_TOOL_SUPPORT: Record<string, (model: string) => boolean | undefined> = {
	...
	openai: (model) => {
		...
		if (model.toLowerCase().startsWith("gpt-4") || model.toLowerCase().startsWith("o3")) {
			return true
		}
	...
	bedrock: (model) => {
		...
		if (
			...
			["claude-3-5", "claude-3.5", "claude-3-7", "claude-3.7"].some((part) => model.toLowerCase().includes(part))
		) {
			return true
		}
	},
	...
	ollama: (model) => {
		...
		if (
			["vision", "math", "guard", "mistrallite", "mistral-openorca"].some((part) =>
				modelName.toLowerCase().includes(part),
			)
		) {
			return false
		}
