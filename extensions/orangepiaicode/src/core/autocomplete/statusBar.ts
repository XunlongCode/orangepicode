...
const statusBarItemText = (status: StatusBarStatus | undefined, loading?: boolean, error?: boolean) => {
	if (error) {
		return "$(alert) OrangePi AI Code (FATAL ERROR)"
	}

	switch (status) {
		case undefined:
			if (loading) {
				return "$(loading~spin) OrangePi AI Code"
			}
		case StatusBarStatus.Disabled:
			return "$(circle-slash) OrangePi AI Code"
		case StatusBarStatus.Enabled:
			return "$(check) OrangePi AI Code"
		case StatusBarStatus.Paused:
			return "$(debug-pause) OrangePi AI Code"
	}
}
...
let statusBarStatusDescription: string = ""

export function setStatusBarDescription(description: string) {
	statusBarStatusDescription = description
}
...
export function setupStatusBar(status: StatusBarStatus | undefined, loading?: boolean, error?: boolean) {
	...
	let text = statusBarItemText(status, loading, statusBarError)

	// 是否展示提示
	if (statusBarStatusDescription) {
		text += `: ${statusBarStatusDescription}`
	}

	statusBarItem.text = text
	statusBarItem.tooltip = statusBarItemTooltip(status ?? statusBarStatus)
	statusBarItem.command = "orangepiaicode.openTabAutocompleteConfigMenu"
...
export function getAutocompleteStatusBarTitle(selected: string | undefined, { title }: ILLM): string {
	if (!title) {
