...
export default class BaseRetrievalPipeline implements IRetrievalPipeline {
	...
	private async initLanceDb() {
		...
		this.lanceDbIndex = await LanceDbIndex.create(embedModel, (uri) => this.options.ide.readFile(uri))
	}

	private getCleanedTrigrams(query: RetrievalPipelineRunArguments["query"]): string[] {
	...
	protected async retrieveFts(args: RetrievalPipelineRunArguments, n: number): Promise<Chunk[]> {
		if (args.query.trim() === "") {
			return []
		}
	...
	protected async retrieveAndChunkRecentlyEditedFiles(n: number): Promise<Chunk[]> {
		const recentlyEditedFilesSlice = Array.from(recentlyEditedFilesCache.keys()).slice(0, n)
	...
	protected async retrieveEmbeddings(input: string, n: number): Promise<Chunk[]> {
