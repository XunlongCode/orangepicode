...
class HttpContextProvider extends BaseContextProvider {
	...
	override get description(): ContextProviderDescription {
		return {
			...
			description: this.options.description || "Retrieve a context item from a custom server",
			type: "normal",
			renderInlineAs: this.options.renderInlineAs || HttpContextProvider.description.renderInlineAs,
		}
	}
