...
export class BaseIssueReporterService extends Disposable {
	...
	public searchVSCodeIssues(title: string, issueDescription?: string): void {
		if (title) {
			this.searchGitHub('XunlongCode/orangepicode', title);
		} else {
			this.clearSearchResults();
		}
	}
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('vscode', "OrangePi Code"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "A OrangePi Code extension"), false));
	...
	public async renderBlocks(): Promise<void> {
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "This extension handles issues outside of OrangePi Code"));
