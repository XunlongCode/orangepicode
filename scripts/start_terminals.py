...
commands = [
    ...
    {"cmd": "yarn watch", "cwd": os.path.join(root_dir, "extensions/orangepiaicode")},
    {
        "cmd": "yarn build && yarn dev",
        "cwd": os.path.join(root_dir, "extensions/orangepiaicode/webview-ui"),
    },
    {
        "cmd": "yarn watch",
        "cwd": os.path.join(root_dir, "extensions/orangepicode-core"),
    },
    {
        "cmd": "yarn build && yarn dev",
        "cwd": os.path.join(root_dir, "extensions/orangepicode-core/webview-ui"),
    },
]
...
def start_terminal_windows(cmd, cwd):
    ...
    try:
        subprocess.Popen(
            f'start cmd.exe /K "cd /d "{cwd}" && {cmd}"', shell=True, cwd=cwd
        )
        print(f"已在新终端中启动: {cmd}")
        return True
