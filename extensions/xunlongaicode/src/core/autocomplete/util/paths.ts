...
const CONTINUE_GLOBAL_DIR = process.env.CONTINUE_GLOBAL_DIR ?? path.join(os.homedir(), ".orangepiaicode")
console.log(CONTINUE_GLOBAL_DIR)
...
export function editConfigJson(callback: (config: SerializedContinueConfig) => SerializedContinueConfig): void {
...
export async function migrate(id: string, callback: () => void | Promise<void>, onAlreadyComplete?: () => void) {
...
export function getConfigJsonPathForRemote(remoteConfigServerUrl: string): string {
	return path.join(getPathToRemoteConfig(remoteConfigServerUrl), "config.json")
}

export function getConfigJsPathForRemote(remoteConfigServerUrl: string): string {
