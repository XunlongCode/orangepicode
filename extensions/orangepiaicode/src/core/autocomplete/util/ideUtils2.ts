...
import type { Range, RangeInFile, Thread } from ".."
...
export class VsCodeIdeUtils {
	...
	showVirtualFile(name: string, contents: string) {
		vscode.workspace
			.openTextDocument(
				vscode.Uri.parse(`${EXTENSION_NAME}:${encodeURIComponent(name)}?${encodeURIComponent(contents)}`),
			)
			.then((doc) => {
	...
	private async _getThreads(session: vscode.DebugSession) {
		...
		const threads = threadsResponse.threads.filter((thread: any) => threadStopped.get(thread.id))
		...
		const variableContext = variablesResponse.variables
			...
			.reduce(
				(acc: any, variable: any) =>
					`${acc}\nname: ${variable.name}, type: ${variable.type}, ` + `value: ${variable.value}`,
				"",
			)
	...
	private static secondsToWaitForGitToLoad = process.env.NODE_ENV === "test" ? 1 : 20
	async getRepo(forDirectory: vscode.Uri): Promise<Repository | undefined> {
		...
		const { foundInDir } = findUriInDirs(forDirectory.toString(), workspaceDirs)
		if (foundInDir) {
