...
export async function createNewPromptFileV2(ide: IDE, promptPath: string | undefined): Promise<void> {
	const workspaceDirs = await ide.getWorkspaceDirs()
	if (workspaceDirs.length === 0) {
		throw new Error("No workspace directories found. Make sure you've opened a folder in your IDE.")
	}

	const baseDirUri = joinPathsToUri(workspaceDirs[0], promptPath ?? ".continue/prompts")
	...
	do {
		...
		promptFileUri = joinPathsToUri(baseDirUri, `new-prompt-file${suffix}.prompt`)
		counter++
