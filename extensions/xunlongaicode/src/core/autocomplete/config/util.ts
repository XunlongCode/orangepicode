...
export function addModel(model: ModelDescription, role?: keyof ExperimentalModelRoles) {
	editConfigJson((config) => {
...
export function addOpenAIKey(key: string) {
	editConfigJson((config) => {
		config.models = config.models
			.filter((model) => model.provider !== "free-trial" || model.model.startsWith("gpt"))
...
export function getModelByRole<T extends keyof ExperimentalModelRoles>(
	...
	const matchingModel = config.models.find((model) => model.title === roleTitle)
