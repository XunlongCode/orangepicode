...
class OpenFilesContextProvider extends BaseContextProvider {
	...
	async getContextItems(query: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
		...
		const openFiles = this.options?.onlyPinned ? await ide.getPinnedFiles() : await ide.getOpenFiles()
		const workspaceDirs = await extras.ide.getWorkspaceDirs()

		return await Promise.all(
			openFiles.map(async (filepath: string) => {
				const content = await ide.readFile(filepath)
				const { relativePathOrBasename, last2Parts, baseName } = getUriDescription(filepath, workspaceDirs)
