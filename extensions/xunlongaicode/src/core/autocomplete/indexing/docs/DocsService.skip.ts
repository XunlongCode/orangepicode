...
describe.skip("DocsService Integration Tests", () => {
	...
	test("Indexing, retrieval, and deletion of a new documentation site", async () => {
		...
		// Lance DB check
		let retrievedChunks = await docsService.retrieveChunksFromQuery("test", mockSiteConfig.startUrl, 5)
		...
		expect(latestConfig.config!.docs).not.toContainEqual(expect.objectContaining(mockSiteConfig))

		// LanceDB check
		retrievedChunks = await docsService.retrieveChunksFromQuery("test", mockSiteConfig.startUrl, 5)
	...
	test("Config synchronization with SQLite", async () => {
		...
		while (docsService.isSyncing) {
			if (Date.now() - startTime > maxWaitTimeMs) {
				throw new Error(`Timeout: docsService.isSyncing did not complete within within ${maxWaitTimeMs}ms`)
			}
