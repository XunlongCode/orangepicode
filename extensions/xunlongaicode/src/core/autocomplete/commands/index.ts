...
export function slashFromCustomCommand(customCommand: CustomCommand): SlashCommand {
	return {
		...
		run: async function* ({ input, llm, history, ide, completionOptions }) {
			...
			if (userInput.startsWith(`/${customCommand.name}`)) {
				userInput = userInput.slice(customCommand.name.length + 1, userInput.length).trimStart()
			}

			// Render prompt template
			let promptUserInput: string
			if (customCommand.prompt.includes("{{{ input }}}")) {
				promptUserInput = await renderTemplatedString(customCommand.prompt, ide.readFile.bind(ide), {
					input: userInput,
				})
			} else {
			...
			for (let i = messages.length - 1; i >= 0; i--) {
				...
				if (
					...
					content.some((part) => "text" in part && part.text?.startsWith(`/${customCommand.name}`))
				) {
					messages[i] = {
						...message,
						content: content.map((part) => {
							if ("text" in part && part.text.startsWith(`/${customCommand.name}`)) {
			...
			for await (const chunk of llm.streamChat(messages, new AbortController().signal, completionOptions)) {
...
export function slashCommandFromDescription(desc: SlashCommandDescription): SlashCommand | undefined {
