...
import { CacheControlEphemeral, ToolUnion } from "@anthropic-ai/sdk/resources"
import {
...
export class AnthropicHandler extends BaseProvider implements SingleCompletionHandler {
	...
	protected async *_createComplete(
		...
		const tools = completionOptions?.tools?.map((tool) => ({
		...
		try {
			const response = await this.client.messages.create(
				{
					messages,
					top_k: completionOptions?.topK,
					top_p: completionOptions?.topP,
					temperature: model.temperature,
					max_tokens: completionOptions?.maxTokens ?? 2048,
					model: model.id,
					stop_sequences: completionOptions?.stop?.filter((x) => x.trim() !== ""),
					stream: false,
					tools,
					tool_choice: completionOptions?.toolChoice
						? {
								type: "tool",
								name: completionOptions?.toolChoice.function.name,
							}
						: undefined,
				},
				{ signal },
			...
			yield renderChatMessage({ role: "assistant", content })
		} catch (error) {
			if (signal.aborted) {
				return
			}
