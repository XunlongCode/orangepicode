...
class Gemini extends BaseLLM {
	...
	protected async *_streamChat(
		...
		const apiBase =
			this.apiBase || Gemini.defaultOptions?.apiBase || "https://generativelanguage.googleapis.com/v1beta/" // Determine if it's a v1 API call based on apiBase
		const isV1API = apiBase.includes("/v1/")

		// Conditionally apply removeSystemMessage
		const convertedMsgs = isV1API ? this.removeSystemMessage(messages) : messages
