...
import { pruneLinesFromBottom, pruneLinesFromTop } from "../../llm/countTokens"
...
export class VerticalDiffManager {
	...
	async streamDiffLines(diffStream: AsyncGenerator<DiffLine>, instant: boolean, streamId: string) {
	...
	async streamEdit(
		...
		if (existingHandler) {
			if (quickEdit) {
				...
				let rangeBool =
					startLine !== endLine || editor.selection.start.character !== editor.selection.end.character

				// Check if the range is different from the previous range
				let newRangeBool =
					startLine !== existingHandler.range.start.line || endLine !== existingHandler.range.end.line
		...
		if (llm.promptTemplates?.apply) {
			...
			overridePrompt = typeof rendered === "string" ? [{ role: "user", content: rendered }] : rendered
		}
