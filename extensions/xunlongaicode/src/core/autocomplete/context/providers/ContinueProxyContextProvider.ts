...
class ContinueProxyContextProvider extends BaseContextProvider {
	...
	override get description(): ContextProviderDescription {
		return {
			...
			displayTitle: this.options.displayTitle || ContinueProxyContextProvider.description.displayTitle,
			description: this.options.description || ContinueProxyContextProvider.description.description,
	...
	async getContextItems(query: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
		...
		const response = await extras.fetch(
			new URL(`/proxy/context/${this.options.id}/retrieve`, env.CONTROL_PLANE_URL),
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
