...
export class ImageFeatureExtractor extends FeatureExtractor {
	...
	async preprocess(
		image,
		{ do_normalize = null, do_pad = null, do_convert_rgb = null, do_convert_grayscale = null } = {},
	) {
...
export class DetrFeatureExtractor extends ImageFeatureExtractor {
	...
	post_process_panoptic_segmentation(
		...
		for (let i = 0; i < batch_size; ++i) {
			...
			let [mask_probs_item, pred_scores_item, pred_labels_item] = this.remove_low_and_no_objects(
				class_logits,
				mask_logits,
				threshold,
				num_labels,
...
export class AutoProcessor {
	...
	static async from_pretrained(
		...
		let preprocessorConfig =
			config ??
			(await getModelJSON(pretrained_model_name_or_path, "preprocessor_config.json", true, {
				progress_callback,
				config,
				cache_dir,
				local_files_only,
