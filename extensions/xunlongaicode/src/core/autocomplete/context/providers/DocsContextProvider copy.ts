...
class DocsContextProvider extends BaseContextProvider {
	...
	private _sortByPreIndexedDocs(submenuItems: ContextSubmenuItem[]): ContextSubmenuItem[] {
	...
	async getContextItems(query: string, extras: ContextProviderExtras): Promise<ContextItem[]> {
		...
		if (useReranking && extras.reranker) {
			chunks = await this._rerankChunks(chunks, extras.reranker, extras.fullInput)
		}
